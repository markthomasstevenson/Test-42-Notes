...
namespace TestNotes.Libraries
{
    ...
    public static class AppBarFunctions
    {
        ...
        private static RegisterInfo GetRegisterInfo(Window appbarWindow, bool formNew = false)
        {
            RegisterInfo reg;
            if (RegisteredWindowInfo.ContainsKey(appbarWindow) && !formNew)
            {
        ...
        {
            var info = GetRegisterInfo(appbarWindow);
            if (info.Edge == ABEdge.None)
            {
                info = GetRegisterInfo(appbarWindow, formNew: true);
            }
            info.Edge = edge;
